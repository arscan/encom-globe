...
class Marker {
  constructor(lat, lon, text, altitude, previous, scene, _opts) {
    ...
    if (this.previous) this.previous.next.push(this)
    ...
    if (this.previous) {
      ...
      for (let j = 0; j < _this.opts.lineSegments + 1; j++) {
        ...
        const nextlat =
          (((90 + previous.lat + j * latdist) % 180) - 90) *
            (0.5 +
              Math.cos((j * ((5 * Math.PI) / 2)) / _this.opts.lineSegments) /
                2) +
          (j * lat) / _this.opts.lineSegments / 2
        const nextlon = ((180 + previous.lon + j * londist) % 360) - 180
        pointList.push({ lat: nextlat, lon: nextlon, index: j })
        if (j == 0 || j == _this.opts.lineSegments) pointList2.push({ lat: nextlat, lon: nextlon, index: j })
        else pointList2.push({ lat: nextlat + 1, lon: nextlon, index: j })
      ...
      update = function () {
        const nextSpot = pointList.shift()
        const nextSpot2 = pointList2.shift()
        ...
        if (pointList.length > 0) setTimeout(update, _this.opts.drawTime / _this.opts.lineSegments)
      }
